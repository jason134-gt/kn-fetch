...
(function ($) {
    $.fn.iChoose = function (options) {
        ...
        var ids=$(iset.chsIds);
        var _l=iset.limit;
		var choFunc = options.choFunc;
        //若无列表数据则中断
        if(iset.data.length==0){return false}
        var isShowMatch = iset.isShowMatch;//初始显示下来框否
        var stylePull = isShowMatch?"display:block;":"display:none;";
        //初始化下拉列表
        var tpl='<div class="'+iset.pullCls+'" style="'+stylePull+'" ><ul>';
        var tplArr=[];
        $.each(iset.data,function(k,v){
            tplArr.push('<li data-mid="'+ v.mid+'" data-name="'+ v.filterName.toUpperCase()+'" class="hook_visible hook_'+v.mid+'" style="display:none">'+ v.name+'</li>');
        });
        ...
        var Action={
            ...
            match:function(str){
                ...
                if(visible.size()==0 && $('.'+iset.noResCls).size()==0){
                    pull.find('li:first').before('<li class="'+iset.noResCls+'">请选择内容</li>')
                }
                this.upstyle('match');
            },
            blur:function(str){

function ThermometerGuage(canvas, options) {
	...
	//唐斌奇 tangbinqi 
    function setTag(name,val,isLeft){
	var max = that.settings.max;
    var min = that.settings.min;
		//if(val < min || val > max)return;
		var ratV = 1-(val-min)/(max-min);//比例值
		if(ratV < 0 ){
			ratV = 0;
			name ="*"+name;
		}
		drawTick( ratV, that.settings.centerTicks, 4, name,isLeft);
    ...
    function drawTick(y, centerTicks, lineWidth, labelText , isLeft /* 增加 */) {
		if(isLeft ==null){isLeft = false;} 
        var ctx = that.ctx;
        var _ = that.settings;
        var w = _.w;
		...
		if(typeof(labelText) == 'number' ){
			...
			ctx.stroke();
			if (labelText != null) {
			  drawTickLabel(cx + (tickWidth * scaleFactor-2) , cy, labelText, -tickWidth,isLeft);
			}
			ctx.restore();
		}else{
			var cx2 = 0;//重构cx值 isLeft == false 将tag的放到右侧
			if(isLeft == false){
				if (_.centerTicks) {
					cx2 = w / 2 + tickWidth / 2;
				} else if (cy > _.topOfOutsideBulb) {
					cx2 = w / 2 + Math.cos(Math.asin((cy - _.bulbVerticalCenter) / bOR)) * bOR + bOR * 0.1;
				} else {
					cx2 = w / 2 - Math.cos(Math.PI * 1.35) * bOR + bOR * 0.1;
				}
				ctx.moveTo(cx2, cy);
				ctx.lineTo(cx2 - (tickWidth * scaleFactor), cy,isLeft);
				ctx.closePath();

				ctx.stroke();
				if (labelText != null) {
				  drawTickLabel(cx2 - (tickWidth * scaleFactor-2)  , cy, labelText, -tickWidth,isLeft);
				}
				ctx.restore();
			}else{
				cx2 = cx;
    ...
    function drawTickLabel(cx, cy, valueText, fontSize,isLeft) {
        var deg, fontSize, metrics, valueText;
		...
		if(typeof(valueText) == 'number'){
			...
			that.ctx.restore();
		}else{//增加支持非数字型 cx此时为右侧时值
			styleText(that.ctx, fontSize.toFixed(0) * that.settings.scaleTickLabelText + 'px sans-serif');
			metrics = measureText(that.ctx, valueText);
			that.ctx.fillStyle = "#000000";
			that.ctx.textBaseline = 'middle';
			if(isLeft == false){
			fillText(that.ctx, valueText, cx, cy);
			}else{
				fillText(that.ctx, valueText, cx - metrics, cy);
			}
			that.ctx.restore();

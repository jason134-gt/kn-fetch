 ...
 * @date：2014-4-10
 */
public class SaveStatisticsHandle implements IHandler {

	static Logger logger = LoggerFactory.getLogger(SaveStatisticsHandle.class);
	...
	static {
		...
		bt.setName("batchSaveStatisticsEventThread");
		bt.start();
		...
		try {
			...
			try {
				...
				for (int x = 0; x < sl.size(); x++) {
					SaveStatisticsEvent v = (SaveStatisticsEvent) sl.get(x);

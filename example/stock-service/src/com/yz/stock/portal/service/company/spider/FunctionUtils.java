...
public class FunctionUtils {
	/** * 匹配超链接的正则表达式 */
	private static String pat = "http://www.oschina.net/code/explore/";
...
//		Matcher matcher = pattern.matcher(url);
		boolean reBoolean = true;// matcher.matches();
		return reBoolean;
	}
	...
	public static void createFile(String content, String urlPath) { 
		...
		for (int i = 1; i < elems.length; i++) {
			...
			if (i == elems.length - 1) {
				...
				boolean test = true;//matcher.matches();
				if (test) {
					if (!file.exists()) {
						file.mkdirs();
					}
...
//		return signContent.substring(start + 1, end);
	...
	public static int isHasGoalContent(String content) {
		int reInt =  1;//content.indexOf("<pre class=\"");
		return reInt;
	}
}

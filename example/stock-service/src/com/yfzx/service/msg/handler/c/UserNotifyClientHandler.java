...
public class UserNotifyClientHandler implements IHandler {

	static Logger logger = LoggerFactory.getLogger(UserNotifyClientHandler.class);
//	private static AtomicInteger _ai = new AtomicInteger(0);// 处理批次
//	private static int batchsize = 100;
	static long lastHandleTimes = System.currentTimeMillis();
	private static UserNotifyClientHandler instance = new UserNotifyClientHandler();
	static String iserviceName = "IRemindService";
	public UserNotifyClientHandler() {

	}

	public static UserNotifyClientHandler getInstance() {
		return instance;
	}
...
//
//		});
//		bt.setName("UserNotifyClientHandler_batchUserNotifyEventThread");
//		bt.start();
...
//		HandleUtil.combineEvent(_bcache,_ai,batchsize,e);
		...
		private void sendSingleMessage(NotifyEvent e) {
			...
			if(openLog){
				logger.info("zjs_send 发送者：" +um.getS()+" 接收者："+um.getD()
						+" 广播方式:" +um.getSendType() +" type:"+um.getMsgType());
			}
			...
			else
			{
				...
				MessageSenderServiceClient.getInstance().sendAsynUdpMessage(seed,e,new ISelectRouter() {
					...
					public String getName() {
						return "SR_UserNotifyClientHandler";
					}
					...
					public Dbrouter selectRouter(String methodName,Object[] args) {
						...
						try {
							...
							boolean openLog = ConfigCenterFactory.getInt("stock_log.msg_center", 0)==1;
							if(openLog){
								logger.info("zjs_send==>" +dr.toString());
							}
		...
		public void sendSyncSingleMessage(NotifyEvent e) {
			UserMsg um = (UserMsg)e.getMsg();
			if(um==null){
				return;
			}
			if(um.getSendType()==MsgConst.SEND_TYPE_1)
			{
				logger.info("同步TPC方式，不支持广播");
				//MessageSenderServiceClient.getInstance().sendAsynUdpMessagePool(e);
			}
			else
			{
				String seed = ((UserMsg) e.getMsg()).getD().toString();
				MessageSenderServiceClient.getInstance().sendSyncMessage(seed, e, new ISelectRouter(){
					@Override
					public String getName() {
						return "SR_UserNotifyClientHandler";
					}
					...
					public Dbrouter selectRouter(String methodName,
						...
						Dbrouter dr = null;
						try {
							if (MessageSenderServiceClient._routerPool.keySet().size() == 0)
								return null;
							String k = args[0].toString();
							if (StringUtil.isEmpty(k))
								return null;
							...
							String ip = selectIp.split(":")[0];
							int port = Integer.valueOf(selectIp.split(":")[1]);
							dr = MessageSenderServiceClient._routerPool.get(getKey(ip, port));
							boolean openLog = ConfigCenterFactory.getInt("stock_log.router_log", 0)==1;
							if(openLog){
								logger.info("zjs_send===> "+dr.toString());
							}

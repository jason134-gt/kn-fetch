 ...
 * @date：2014-3-25
 */
public class HWeekAvgService {

	private static HWeekAvgService instance = new HWeekAvgService();
	Logger log = LoggerFactory.getLogger(this.getClass());
	PLayerEnter pLayerEnter = BaseFactory.getPLayerEnter();

	private HWeekAvgService() {

	}

	public static HWeekAvgService getInstance() {
		return instance;
	}
	 ...
	 * @return
	 */
	public Double computeWeekAvg(IndexMessage req, int type) {
		...
		int count=0;
		Date mintime = USubjectService.getInstance().getTradeIndexMinTime(treq.getUidentify(),StockConstants.INDEX_CODE_TRADE_S_W);
		...
		Cfirule cr = CRuleService.getInstance().getCfruleByCodeFromCache(StockConstants.INDEX_CODE_TRADE_S_W);
		while ( count<type&&c.getTime().compareTo(mintime) >= 0
			...
			if(actime!=null)
			{
				...
				if (treq.isNeedUseExtDataCache()) {
					r = IndexValueAgent.getIndexValue(treq.getCompanyCode(),
							StockConstants.INDEX_CODE_TRADE_S_W, treq.getTime());
				}
				//取不到就计算
